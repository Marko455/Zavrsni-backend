...
def login_seller(seller: SellerLogin, db: Session = Depends(get_db)):
    ...
    return {"message": "Login successful", "seller": db_seller.name}
...
def login_buyer(buyer: BuyerLogin, db: Session = Depends(get_db)):
    ...
    if not db_buyer or not verify_password(buyer.password, db_buyer.password):
        raise HTTPException(status_code=400, detail="Invalid email or password")
    return {"message": "Login successful", "buyer": db_buyer.name}
...
def create_property(property: PropertyCreate, db: Session = Depends(get_db)):
    ...
    index.add_with_ids(np.array([embedding], dtype=np.float32), np.array([db_property.id], dtype=np.int64))
...
def generate_and_execute_sql(
    ...
    # 1. Generiranje SQL upita koristeći Llamu
    prompt = f"""
    You are an expert SQL assistant. Generate an SQL query based on the user's question and the following database schema:
